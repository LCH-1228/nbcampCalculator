...
public class AddOperation:Operation {
    ...
    // Operation 프로토콜에서 정의된 calculate의 구현부로 입력된 두 파라미터의 타입이 모두 BinaryInteger 또는 BinaryFloatingPoint로 동일한 경우에 더하기 연산
    public func calculate<T>(_ firstNumber: T, _ secondNumber: T) throws where T: Numeric {
        print(firstNumber + secondNumber)
    }
    
    // Operation 프로토콜에서 정의된 calculate의 구현부로 입력된 첫번째 파라미터의 타입이 BinaryInteger이고 두번째 파라미터의 타입이 BinaryFloatingPoint 경우에 더하기 연산
    ...
    // Operation 프로토콜에서 정의된 calculate의 구현부로 입력된 첫번째 파라미터의 타입이 BinaryFloatingPoint이고 두번째 파라미터의 타입이 BinaryInteger 경우에 더하기 연산
